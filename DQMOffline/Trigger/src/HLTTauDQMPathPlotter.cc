...
#include <algorithm>
void HLTTauDQMPathPlotter::bookHistograms(DQMStore::IBooker& iBooker, PlotLevel* pl) {
  if (!isValid())
    return;
  ...
  hAcceptedEvents_ = pl->book(iBooker.book1D(
  ...
  if (doRefAnalysis_) {
    ...
    if (hltPath_.hasL2Taus()) {
      hL2TrigTauEtEffNum_ = pl->book(iBooker.book1D("L2TrigTauEtEffNum", "L2 #tau p_{T} efficiency;Ref #tau p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hL2TrigTauEtEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigTauEtEffDenom", "L2 #tau p_{T} denominator;Ref #tau p_{T};Efficiency", ptbins_, 0, ptmax_),kEverything);
      hL2TrigTauEtaEffNum_ = pl->book(iBooker.book1D(
          "L2TrigTauEtaEffNum", "L2 #tau #eta efficiency;Ref #tau #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hL2TrigTauEtaEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigTauEtaEffDenom", "L2 #tau #eta denominator;Ref #tau #eta;Efficiency", etabins_, -2.5, 2.5),kEverything);
      hL2TrigTauPhiEffNum_ = pl->book(iBooker.book1D(
          "L2TrigTauPhiEffNum", "L2 #tau #phi efficiency;Ref #tau #phi;entries", phibins_, -3.2, 3.2),kEverything);
      hL2TrigTauPhiEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigTauPhiEffDenom", "L2 #tau #phi denominator;Ref #tau #phi;Efficiency", phibins_, -3.2, 3.2),kEverything);
      hL2TrigTauHighEtEffNum_ = pl->book(iBooker.book1D("L2TrigTauHighEtEffNum",
                                               "L2 #tau p_{T} efficiency (high p_{T});Ref #tau p_{T};entries",
                                               ptbins_,
                                               0,
                                               highptmax_),kEverything);
      hL2TrigTauHighEtEffDenom_ = pl->book(iBooker.book1D("L2TrigTauHighEtEffDenom",
    ...
    if (hltPath_.hasL3Taus()) {
      hL3TrigTauEtEffNum_ = pl->book(iBooker.book1D(
          "L3TrigTauEtEffNum", "L3 #tau p_{T} efficiency;Ref #tau p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hL3TrigTauEtEffDenom_ = pl->book(iBooker.book1D(
          "L3TrigTauEtEffDenom", "L3 #tau p_{T} denominator;Ref #tau p_{T};Efficiency", ptbins_, 0, ptmax_),kEverything);
      hL3TrigTauEtaEffNum_ = pl->book(iBooker.book1D(
          "L3TrigTauEtaEffNum", "L3 #tau #eta efficiency;Ref #tau #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hL3TrigTauEtaEffDenom_ = pl->book(iBooker.book1D(
          "L3TrigTauEtaEffDenom", "L3 #tau #eta denominator;Ref #tau #eta;Efficiency", etabins_, -2.5, 2.5),kEverything);
      hL3TrigTauPhiEffNum_ = pl->book(iBooker.book1D(
          "L3TrigTauPhiEffNum", "L3 #tau #phi efficiency;Ref #tau #phi;entries", phibins_, -3.2, 3.2),kEverything);
      hL3TrigTauPhiEffDenom_ = pl->book(iBooker.book1D(
          "L3TrigTauPhiEffDenom", "L3 #tau #phi denominator;Ref #tau #phi;Efficiency", phibins_, -3.2, 3.2),kEverything);
      hL3TrigTauHighEtEffNum_ = pl->book(iBooker.book1D("L3TrigTauHighEtEffNum",
                                               "L3 #tau p_{T} efficiency (high p_{T});Ref #tau p_{T};entries",
                                               ptbins_,
                                               0,
					       highptmax_),kEverything);
      hL3TrigTauHighEtEffDenom_ = pl->book(iBooker.book1D("L3TrigTauHighEtEffDenom",
                                                 "L3 #tau p_{T} denominator (high p_{T});Ref #tau p_{T};Efficiency",
                                                 ptbins_,
                                                 0,
						 highptmax_),kEverything);
      hL3TrigTauEtaPhiEffNum_ = pl->book(iBooker.book2D(
          "L3TrigTauEtaPhiEffNum", "L3 efficiency in eta-phi plane", etabins_, -2.5, 2.5, phibins_, -3.2, 3.2),kEverything);
      hL3TrigTauEtaPhiEffDenom_ = pl->book(iBooker.book2D(
          "L3TrigTauEtaPhiEffDenom", "L3 denominator in eta-phi plane", etabins_, -2.5, 2.5, phibins_, -3.2, 3.2),kEverything);
      if(hL3TrigTauEtaPhiEffDenom_) hL3TrigTauEtaPhiEffDenom_->getTH2F()->SetOption("COL");
    }

    if (hltPath_.hasL2Electrons()) {
      hL2TrigElectronEtEffNum_ = pl->book(iBooker.book1D(
          "L2TrigElectronEtEffNum", "L2 electron p_{T} efficiency;Ref electron p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hL2TrigElectronEtEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigElectronEtEffDenom", "L2 electron p_{T} denominator;Ref electron p_{T};Efficiency", ptbins_, 0, ptmax_),kEverything);
      hL2TrigElectronEtaEffNum_ = pl->book(iBooker.book1D(
          "L2TrigElectronEtaEffNum", "L2 electron #eta efficiency;Ref electron #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hL2TrigElectronEtaEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigElectronEtaEffDenom", "L2 electron #eta denominator;Ref electron #eta;Efficiency", etabins_, -2.5, 2.5),kEverything);
      hL2TrigElectronPhiEffNum_ = pl->book(iBooker.book1D(
	  "L2TrigElectronPhiEffNum", "L2 electron #phi efficiency;Ref electron #phi;entries", phibins_, -3.2, 3.2),kEverything);
      hL2TrigElectronPhiEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigElectronPhiEffDenom", "L2 electron #phi denominator;Ref electron #phi;Efficiency", phibins_, -3.2, 3.2),kEverything);
    }

    if (hltPath_.hasL3Electrons()) {
      hL3TrigElectronEtEffNum_ = pl->book(iBooker.book1D(
          "L3TrigElectronEtEffNum", "L3 electron p_{T} efficiency;Ref electron p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hL3TrigElectronEtEffDenom_ = pl->book(iBooker.book1D(
          "L3TrigElectronEtEffDenom", "L3 electron p_{T} denominator;Ref electron p_{T};Efficiency", ptbins_, 0, ptmax_),kEverything);
      hL3TrigElectronEtaEffNum_ = pl->book(iBooker.book1D(
	  "L3TrigElectronEtaEffNum", "L3 electron #eta efficiency;Ref electron #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hL3TrigElectronEtaEffDenom_ = pl->book(iBooker.book1D(
          "L3TrigElectronEtaEffDenom", "L3 electron #eta denominator;Ref electron #eta;Efficiency", etabins_, -2.5, 2.5),kEverything);
      hL3TrigElectronPhiEffNum_ = pl->book(iBooker.book1D(
	  "L3TrigElectronPhiEffNum", "L3 electron #phi efficiency;Ref electron #phi;entries", phibins_, -3.2, 3.2),kEverything);
      hL3TrigElectronPhiEffDenom_ = pl->book(iBooker.book1D(
    ...
    if (hltPath_.hasL2Muons()) {
      hL2TrigMuonEtEffNum_ = pl->book(iBooker.book1D(
          "L2TrigMuonEtEffNum", "L2 muon p_{T} efficiency;Ref muon p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hL2TrigMuonEtEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigMuonEtEffDenom", "L2 muon p_{T} denominator;Ref muon p_{T};Efficiency", ptbins_, 0, ptmax_),kEverything);
      hL2TrigMuonEtaEffNum_ = pl->book(iBooker.book1D(
          "L2TrigMuonEtaEffNum", "L2 muon #eta efficiency;Ref muon #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hL2TrigMuonEtaEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigMuonEtaEffDenom", "L2 muon #eta denominator;Ref muon #eta;Efficiency", etabins_, -2.5, 2.5),kEverything);
      hL2TrigMuonPhiEffNum_ = pl->book(iBooker.book1D(
          "L2TrigMuonPhiEffNum", "L2 muon #phi efficiency;Ref muon #phi;entries", phibins_, -3.2, 3.2),kEverything);
      hL2TrigMuonPhiEffDenom_ = pl->book(iBooker.book1D(
          "L2TrigMuonPhiEffDenom", "L2 muon #phi denominator;Ref muon #phi;Efficiency", phibins_, -3.2, 3.2),kEverything);
    }

    if (hltPath_.hasL3Muons()) {
      hL3TrigMuonEtEffNum_ = pl->book(iBooker.book1D(
           "L3TrigMuonEtEffNum", "L3 muon p_{T} efficiency;Ref muon p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hL3TrigMuonEtEffDenom_ = pl->book(iBooker.book1D(
           "L3TrigMuonEtEffDenom", "L3 muon p_{T} denominator;Ref muon p_{T};Efficiency", ptbins_, 0, ptmax_),kEverything);
      hL3TrigMuonEtaEffNum_ = pl->book(iBooker.book1D(
           "L3TrigMuonEtaEffNum", "L3 muon #eta efficiency;Ref muon #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hL3TrigMuonEtaEffDenom_ = pl->book(iBooker.book1D(
          "L3TrigMuonEtaEffDenom", "L3 muon #eta denominator;Ref muon #eta;Efficiency", etabins_, -2.5, 2.5),kEverything);
      hL3TrigMuonPhiEffNum_ = pl->book(iBooker.book1D(
	  "L3TrigMuonPhiEffNum", "L3 muon #phi efficiency;Ref muon #phi;entries", phibins_, -3.2, 3.2),kEverything);
      hL3TrigMuonPhiEffDenom_ = pl->book(iBooker.book1D(
    ...
    if (hltPath_.hasL2CaloMET()) {
      hL2TrigMETEtEffNum_ =
          pl->book(iBooker.book1D("L2TrigMETEtEffNum", "L2 MET efficiency;Ref MET;entries", ptbins_, 0, ptmax_),kEverything);
      hL2TrigMETEtEffDenom_ =
          pl->book(iBooker.book1D("L2TrigMETEtEffDenom", "L2 MET denominator;Ref MET;Efficiency", ptbins_, 0, ptmax_),kEverything);
    }
  ...
  if (doRefAnalysis_) {
    ...
    auto create = [&](const std::string& name) {
      if (name == "tau-met") {
        hMass_ = pl->book(iBooker.book1D(
	    "ReferenceMass", "Transverse mass of reference " + name + ";Reference transverse mass;entries", 100, 0, 500),kEverything);
      } else {
        hMass_ = pl->book(iBooker.book1D(
            "ReferenceMass", "Invariant mass of reference " + name + ";Reference invariant mass;entries", 100, 0, 500),kEverything);
      }
    };
    LogDebug("HLTTauDQMOffline") << "Path " << hltPath_.getPathName() << " number of taus " << ntaus << " electrons "
                                 << neles << " muons " << nmus;
    if (ntaus > 0) {
      hTrigTauEt_ = pl->book(iBooker.book1D("TrigTauEt", "Triggered #tau p_{T};#tau p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hTrigTauEta_ = pl->book(iBooker.book1D("TrigTauEta", "Triggered #tau #eta;#tau #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hTrigTauPhi_ = pl->book(iBooker.book1D("TrigTauPhi", "Triggered #tau #phi;#tau #phi;entries", phibins_, -3.2, 3.2),kEverything);
    }
    if (neles > 0) {
      hTrigElectronEt_ =
        pl->book(iBooker.book1D("TrigElectronEt", "Triggered electron p_{T};electron p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hTrigElectronEta_ =
        pl->book(iBooker.book1D("TrigElectronEta", "Triggered electron #eta;electron #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hTrigElectronPhi_ =
        pl->book(iBooker.book1D("TrigElectronPhi", "Triggered electron #phi;electron #phi;entries", phibins_, -3.2, 3.2),kEverything);
    }
    if (nmus > 0) {
      hTrigMuonEt_ = pl->book(iBooker.book1D("TrigMuonEt", "Triggered muon p_{T};muon p_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hTrigMuonEta_ = pl->book(iBooker.book1D("TrigMuonEta", "Triggered muon #eta;muon #eta;entries", etabins_, -2.5, 2.5),kEverything);
      hTrigMuonPhi_ = pl->book(iBooker.book1D("TrigMuonPhi", "Triggered muon #phi;muon #phi;entries", phibins_, -3.2, 3.2),kEverything);
    }
    if (nmet > 0) {
      hTrigMETEt_ = pl->book(iBooker.book1D("TrigMETEt", "Triggered MET E_{T};MET E_{T};entries", ptbins_, 0, ptmax_),kEverything);
      hTrigMETPhi_ = pl->book(iBooker.book1D("TrigMETPhi", "Triggered MET #phi;MET #phi;entries", phibins_, -3.2, 3.2),kEverything);
    }
